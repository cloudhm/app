...
#define kTabOffset 0
#define kTabWidth CGRectGetWidth([UIScreen mainScreen].bounds)/3+1.5f
...
//    [bezierPath stroke];
    ...
    // Draw side line (add)
//    bezierPath = [UIBezierPath bezierPath];
//    [bezierPath moveToPoint:CGPointMake(0.f, 10.f)];
//    [bezierPath addLineToPoint:CGPointMake(0.f, CGRectGetHeight(rect) - 10.f)];
//    [[UIColor colorWithRed:118/255.f green:118/255.f blue:118/255.f alpha:1]setStroke];
//    [bezierPath setLineWidth:1.0];
//    [bezierPath stroke];
    
    bezierPath = [UIBezierPath bezierPath];
    [bezierPath moveToPoint:CGPointMake(CGRectGetMaxX(rect)+1.f, 10.f)];
    [bezierPath addLineToPoint:CGPointMake(CGRectGetMaxX(rect)+1.f, CGRectGetHeight(rect) - 10.f)];
    [[UIColor colorWithRed:118/255.f green:118/255.f blue:118/255.f alpha:1]setStroke];
    ...
    if (self.selected) {
        ...
        // Draw the indicator
        [bezierPath moveToPoint:CGPointMake(5.f, CGRectGetHeight(rect) - 1.0)];
        [bezierPath addLineToPoint:CGPointMake(CGRectGetWidth(rect)-5.f, CGRectGetHeight(rect) - 1.0)];
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    ...
    if ([activeTabView.subviews.lastObject isMemberOfClass:[UILabel class]]) {//作一下逻辑判断，该视图是否为UILabel类实例
        label = activeTabView.subviews.lastObject;//内部封装设置选中的颜色
    ...
    if ([activeTabView.subviews.lastObject isMemberOfClass:[UILabel class]]) {
        label = activeTabView.subviews.lastObject;
    ...
    if ([self.centerCurrentTab boolValue]) {
        ...
        frame.size.width = CGRectGetWidth(self.tabsView.frame) - 2.5f;//减少可见的滚动区域1个像素，画的边就看不到了😄
    }
...
- (void)defaultSetup {
    ...
    if (!self.tabsView) {
        ...
        self.tabsView.bounces = NO;//add no bounces
